...
describe('WordPress发布Lambda函数', () => {
  ...
  describe('请求验证', () => {
    it('当请求体为空时应返回错误', async () => {
      ...
      expect(JSON.parse(result.body).error).toBe('Request body cannot be empty');
    });
    ...
    it('当JSON格式无效时应返回错误', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).error).toBe('Invalid request body JSON format');
    });
    ...
    it('当URL缺失时应返回错误', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).error).toBe('WordPress URL(url) cannot be empty');
    });
    ...
    it('当缺少必填字段username时应返回错误', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).error).toBe('Username(username) cannot be empty');
    });
    ...
    it('当缺少必填字段password时应返回错误', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).error).toBe('Password(password) cannot be empty');
    });
    ...
    it('当URL格式无效时应返回错误', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).error).toBe('Invalid WordPress URL format');
    });
    ...
    it('当关键词为空数组时应返回错误', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).error).toContain('Keywords(keywords) must be a non-empty array');
    });
    ...
    it('当内容提示为空时应返回错误', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).error).toContain('Content prompt(prompt) cannot be empty');
    });
  ...
  describe('WordPress文章发布', () => {
    it('应成功创建WordPress文章', async () => {
      ...
      expect(JSON.parse(result.body).message).toBe('Article published successfully');
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          status: 'publish',
          tags_input: ['test', 'blog']  // 将 tags 改为 tags_input
        }),
        expect.objectContaining({
          auth: {
            username: 'test_user',
    ...
    it('应处理有特殊字符的标签', async () => {
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({
          tags_input: ['test1', 'test2']  // 将 tags 改为 tags_input
        }),
        expect.any(Object)
      );
    });
  ...
  describe('错误处理', () => {
    it('应处理401认证失败错误', async () => {
      ...
      expect(JSON.parse(result.body).error).toContain('WordPress authentication failed');
    });
    ...
    it('应处理403权限不足错误', async () => {
      ...
      expect(JSON.parse(result.body).error).toContain('Insufficient permissions');
    });
    ...
    it('应处理404端点未找到错误', async () => {
      ...
      expect(JSON.parse(result.body).error).toContain('WordPress API endpoint not found');
    });
